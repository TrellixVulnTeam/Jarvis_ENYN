...
def __schema_to_waiting_notification_list(schema: list[WaitingNotification]) -> list[str]:
    return [w.text for w in schema]


def __waiting_notification_list_to_schema(user_id: int, models: list[str]) -> list[WaitingNotification]:
    return [WaitingNotification(user_id=user_id, text=w) for w in models]


def schema_to_user(schema: UserSchema) -> User:
    return User(
        uid=schema.id,
        alias=schema.alias,
        ...
        waiting_notifications=__schema_to_waiting_notification_list(schema.waiting_notifications)
    )
...
def user_to_schema(model: User) -> UserSchema:
    return UserSchema(
        ...
        waiting_notifications=__waiting_notification_list_to_schema(model.uid, model.waiting_notifications)
    )
