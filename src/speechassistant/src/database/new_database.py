...
from src.database.connections.AbstractDataBaseConnection import (
    AbstractDataBaseConnection, Model, Schema,
)
from src.database.schemas.alarmSchema import (
    AlarmSchema,
    alarm_to_schema,
    schema_to_alarm,
)
from src.database.schemas.audioFileSchema import audio_file_to_schema, schema_to_audio_file, \
    AudioFileSchema
from src.database.schemas.birthdaySchema import BirthdaySchema, schema_to_birthday, birthday_to_schema
from src.database.schemas.reminderSchema import ReminderSchema, schema_to_reminder, reminder_to_schema
from src.database.schemas.routineSchema import RoutineSchema, schema_to_routine, routine_to_schema
from src.database.schemas.shoppingListSchema import ShoppingListSchema, schema_to_shopping_list, \
    shopping_list_to_schema
from src.database.schemas.timerSchema import TimerSchema, schema_to_timer, timer_to_schema
from src.database.schemas.userSchema import UserSchema, schema_to_user, user_to_schema
...
class _AlarmInterface(AbstractDataBaseConnection[Alarm, AlarmSchema]):
    ...
    def __init__(self):
        ...
        from src.database.tables.alarmTables import create_tables
...
class _AudioFileInterface(AbstractDataBaseConnection[AudioFile, AudioFileSchema]):
    ...
    def __int__(self) -> None:
        super().__init__()
        from src.database.tables.audiofileTable import create_tables
        create_tables(self.engine)
...
class _BirthdayInterface(AbstractDataBaseConnection[Birthday, BirthdaySchema]):
    ...
    def __int__(self) -> None:
        super().__init__()
        from src.database.tables.birthdayTable import create_tables
        create_tables(self.engine)
...
class _ReminderInterface(AbstractDataBaseConnection[Reminder, ReminderSchema]):
    ...
    def __int__(self) -> None:
        super().__init__()
        from src.database.tables.reminderTable import create_tables
        create_tables(self.engine)
...
class _RoutineInterface(AbstractDataBaseConnection[Routine, RoutineSchema]):
    ...
    def __int__(self) -> None:
        super().__init__()
        from src.database.tables.routineTable import create_tables
        create_tables(self.engine)
...
class _ShoppingListInterface(AbstractDataBaseConnection[ShoppingListItem, ShoppingListSchema]):
    ...
    def __int__(self) -> None:
        super().__init__()
        from src.database.tables.shoppinglistTable import create_tables
        create_tables(self.engine)
...
class _TimerInterface(AbstractDataBaseConnection[Timer, TimerSchema]):
    ...
    def __int__(self) -> None:
        super().__init__()
        from src.database.tables.timerTable import create_tables
        create_tables(self.engine)
...
class _UserInterface(AbstractDataBaseConnection[User, UserSchema]):
    ...
    def __int__(self) -> None:
        super().__init__()
        from src.database.tables.userTable import create_tables
        create_tables(self.engine)
