...
def read_routine(name: str | None) -> Response:
    if name is not None:
            ...
            preparedData.append(
                {
                    ...
                    "after_sunrise": routine["retakes"]["activation"].get(
                        "after_sunrise"
                    ),
                    "after_sunset": routine["retakes"]["activation"].get(
                        "after_sunset"
                    ),
        ...
        return Response(
            json.dumps(preparedData), mimetype="application/json", status=200
        )


def update_routine(data: dict) -> Response:
    database.routine_interface.update_routine(
        old_name=data["name"], new_routine_dict=data
    )
    return Response(
        f'Routine with name {data["rname"]} updated successfully!', status=200
    )
...
def create_routine_command(rname: str, module_name: str, commands: list) -> Response:
    rcid: int = database.routine_interface.create_routine_commands(
        rname, module_name, commands
    )
...
def create_routine_time(rname, hour, minute) -> Response:
    ratid: int = database.routine_interface.create_routine_activation_time(
        rname, hour, minute
    )
