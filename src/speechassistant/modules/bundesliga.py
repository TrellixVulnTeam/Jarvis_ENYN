...
vereinPattern = re.compile(r'auf (welchem platz|welchem rang|welcher platzierung|welcher position) ist (.+?) in (.+)',
                           re.I)
...
def getLigaId(txt):
    ...
    if 'bundesliga' in text:
        ...
        return ['cl', 'cl-' + year, 'cl' + year, 'cl-' + year[2:], 'cl' + year[2:], 'ucl-' + year, 'ucl' + year,
                'ucl-' + year[2:], 'ucl' + year[2:]]
        ...
        if 'weltmeisterschaft' in text or ' wm ' in text:
            ...
            year = str(datetime.datetime.now().year)
            return ['em-' + year, 'em' + year, 'em-' + year[2:], 'em' + year[2:], 'uefa-euro--' + year,
                    'uefa-euro-' + year]
    else:
        return None
        ...
        for t in range(len(js)):
            try:
                if js[t]['TeamName'].lower() in verein.lower() or (
                        'ShortName' in js[t] and js[t]['ShortName'] is not None and js[t][
                    'ShortName'].lower() in verein.lower()):
                    team = js[t]
                    teamRang = t + 1
            except (IndexError, KeyError):
                hadExc = True
...
def isValid(text):
    ...
    return 'bundesliga' in text or \
           (('fussball' in text or 'fußball' in text) and (
                       'weltmeisterschaft' in text or ' wm ' in text or 'europameisterschaft' in text or 'europa meisterschaft' in text or ' em ' in text)) or \
