...
def Webserver(_core, _temp_module_wrapper):
    ...
    helpModuleWrapper = _temp_module_wrapper
    ...
    def controlPhilipsLights():
        established = True if (not _core.local_storage["module_storage"]["philips_hue"]["Bridge-Ip"] == "") else False
    ...
    def controlAlarm():
        ...
        reg_len = 0
        sin_len = 0
        for item in _core.local_storage["alarm"]["regular"]:
            reg_len += len(_core.local_storage["alarm"]["regular"][item])
        for item in _core.local_storage["alarm"]["regular"]:
            sin_len += len(_core.local_storage["alarm"]["single"][item])
    ...
    def editModule(moduleName):
        with open(_core.path + '/modules/' + moduleName + '.py', 'r') as file:
            fileCode = file.read()
    ...
    def serve_content(svgFile):
        return send_file(_core.path + '/webserver/static/svg/weatherIcons/' + svgFile + '.svg')
    ...
    def writeSystemConfig():
        data = getData()
        config_data = _core.config_data
        config_data["Local_storage"] = _core.local_storage
    ...
    def loadUserConfig(userName):
        if os.path.exists("./users/" + userName + "/data.json"):
            ...
            data = {
                ...
                "telegram_established": True if _core.config_data["messenger_key"] != "" else False
            }
            return jsonify(data)
        else:
    ...
    def changeModuleMode(modName, action):
        modules = _core.local_storage["modules"]  # dummyList
        if modName in modules:
            if action == "load":
                ...
                newCode = getData()
                with open(_core.path + "/modules/" + modName + ".py", 'w') as file:
                    file.write(newCode)
        else:
    ...
    def getAlarmSound(filename):
        def gen():
            with open(_core.path + "/modules/resources/alarm_sounds/" + filename, "rb") as fwav:
                data = fwav.read(1024)
                while data:
                    yield data
                    data = fwav.read(1024)
    ...
    def listAlarm(action="*"):
        ...
        for item in _core.local_storage["alarm"]["regular"]:
            if not _core.local_storage["alarm"]["regular"][item] == []:
                regular_present = True
                break
        for item in _core.local_storage["alarm"]["single"]:
            if not item is []:
                single_present = True
                break

        temp_regular = _core.local_storage["alarm"]["regular"]
        temp_single = _core.local_storage["alarm"]["single"]
    ...
    def changeAlarm(action, regular, day, hour, minute):
        ...
        if not (day.capitalize() in _core.skills.Statics.weekdays_engl):
            day = _core.skills.Statics.weekdays_ger_to_eng[day.lower()]
        day = day.lower()
        if action == "delete":
    ...
    def get_alarms_german(repeat):
        new_list = {}
        for day in _core.skills.Statics.weekdays:
            new_list[day] = _core.local_storage["alarm"][repeat][_core.skills.Statics.weekdays_ger_to_eng[day.lower()]]
        return new_list
