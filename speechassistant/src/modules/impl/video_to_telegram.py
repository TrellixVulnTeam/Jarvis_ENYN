...
def handle(text, core, skills):
    ...
    try:
        core.say(
            "Video wird heruntergeladen. Bitte warte einen Moment.", output="messenger"
        )
        try:
...
def send_video_to_messenger(core, VIDEO_PATH):
    try:
        ...
        core.messenger.bot.send_file(
            uid, video=open(VIDEO_PATH, "rb"), supports_streaming=True
        )
