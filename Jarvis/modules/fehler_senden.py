...
def handle(text, core, skills):
	core.say("Wilkommen in dem Modul, um einen Fehler zu senden. Wenn du nicht mehr weiter machen möchtest, dann sag einfach 'abbruch'.")
	...
	if name is 'abbruch':
		...
		if modul_name is 'abbruch':
			...
			if mail is 'abbruch':
				...
				core.say("Okay gut. Nun beschreib den Fehler bitte ausführlicher. Ab jetzt kannst du leider den Vorgang nicht mehr abbrechen. ")
				description = core.listen()
			
			
			sendIssu(name, modul_name, mail, description)
			core.say('Vielen Dank! Ich werde den Fehler an Jakob weiterleiten. Bitte schick auch in Zukunft Fehler, damit Core weiterhin gut genutzt werden kann.')
