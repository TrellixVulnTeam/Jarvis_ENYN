...
def run(core, profile):
    ...
    users = core.local_storage["users"]
    ...
    for user in users:
        ...
        with open(core.path + '/resources/users/' + user[name] + '/other_informations/birthdays.json', 'r') as config_file:
        ...
        for item in foreign_birthdays:
            ...
            day = int(geburtsdatum['day'])
            if now.month == month and now.day == day:
                for user in core.local_storage["users"]:
