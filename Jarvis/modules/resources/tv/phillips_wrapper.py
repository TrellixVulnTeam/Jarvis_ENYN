...
class Pylips:
        ...
        if (len(sys.argv) == 1 or (len(sys.argv) == 3 and sys.argv[1] == "--config")) and self.config["DEFAULT"][
            ...
            if len(self.config["MQTT"]["host"]) > 0:
                ...
                if self.config["DEFAULT"]["mqtt_update"] == "True":
                    ...
                    self.last_status = {"powerstate": None, "volume": None, "muted": False, "cur_app": None,
    ...
    def find_api_version(self, verbose=True, possible_ports=[1925], possible_api_versions=[6, 5, 1]):
        ...
        for port in possible_ports:
            for api_version in possible_api_versions:
                try:
                    if verbose:
                        print("Trying", str(protocol) + str(self.config["TV"]["host"]) + ":" + str(port) + "/" + str(
                            api_version) + "/system")
                    r = session.get(str(protocol) + str(self.config["TV"]["host"]) + ":" + str(port) + "/" + str(
    ...
    def pair_request(self, data, err_count=0):
        ...
        response = {}
        r = session.post(
            "https://" + str(self.config["TV"]["host"]) + ":1926/" + str(self.config["TV"]["apiv"]) + "/pair/request",
        ...
        auth["auth_signature"] = self.create_signature(b64decode(secret_key),
    ...
    def pair_confirm(self, data, err_count=0):
        while err_count < 10:
            ...
            try:
                r = session.post("https://" + str(self.config["TV"]["host"]) + ":1926/" + str(
                    self.config["TV"]["apiv"]) + "/pair/grant", json=data, verify=False,
                                 auth=HTTPDigestAuth(self.config["TV"]["user"], self.config["TV"]["pass"]), timeout=2)
        ...
        while err_count < int(self.config["DEFAULT"]["num_retries"]):
            if verbose:
                print("Sending GET request to",
                      str(self.config["TV"]["protocol"]) + str(self.config["TV"]["host"]) + ":" + str(
                          self.config["TV"]["port"]) + "/" + str(self.config["TV"]["apiv"]) + "/" + str(path))
            try:
                r = session.get(str(self.config["TV"]["protocol"]) + str(self.config["TV"]["host"]) + ":" + str(
                    self.config["TV"]["port"]) + "/" + str(self.config["TV"]["apiv"]) + "/" + str(path), verify=False,
            ...
            if self.config["DEFAULT"]["mqtt_listen"].lower() == "true":
                self.mqtt_update_status(
                    {"powerstate": "Off", "volume": None, "muted": False, "cur_app": None, "ambilight": None,
                     "ambihue": False})
            ...
            if verbose:
                print("Sending POST request to",
                      str(self.config["TV"]["protocol"]) + str(self.config["TV"]["host"]) + ":" + str(
                          self.config["TV"]["port"]) + "/" + str(self.config["TV"]["apiv"]) + "/" + str(path))
            try:
                r = session.post(str(self.config["TV"]["protocol"]) + str(self.config["TV"]["host"]) + ":" + str(
                    self.config["TV"]["port"]) + "/" + str(self.config["TV"]["apiv"]) + "/" + str(path), json=body,
                                 verify=False,
                                 auth=HTTPDigestAuth(str(self.config["TV"]["user"]), str(self.config["TV"]["pass"])),
                                 timeout=2)
            except Exception:
        ...
        if ambihue_status is not None and ambihue_status[0] == '{':
            ...
            if "power" in ambihue_status:
                ...
                if self.last_status["ambihue"] != ambihue:
                    self.mqtt.publish(str(self.config["MQTT"]["topic_pylips"]),
                                      json.dumps({"status": {"ambihue": ambihue}}), retain=False)
    ...
    def mqtt_update_app(self):
        ...
        if actv_status is not None and actv_status[0] == '{':
            ...
            if "component" in actv_status:
                if actv_status["component"]["packageName"] == "org.droidtv.zapster" or actv_status["component"][
                    "packageName"] == "NA":
                    self.mqtt_update_channel()
                else:
                    if self.last_status["cur_app"] is None or self.last_status["cur_app"] != actv_status["component"][
                        "packageName"]:
                        self.mqtt.publish(str(self.config["MQTT"]["topic_pylips"]),
                                          json.dumps({"status": {"cur_app": actv_status["component"]["packageName"]}}),
                                          retain=False)
        ...
        if channel is not None and channel[0] == '{':
            ...
            if "channel" in channel:
                if json.dumps(self.last_status["cur_app"]) != json.dumps({"app": "TV", "channel": channel}):
                    self.mqtt.publish(str(self.config["MQTT"]["topic_pylips"]),
                                      json.dumps({"status": {"cur_app": {"app": "TV", "channel": channel}}}),
                                      retain=False)
        ...
        if vol_status is not None:
            ...
            if "muted" in vol_status:
                ...
                if self.last_status["muted"] != muted or self.last_status["volume"] != volume:
                    self.mqtt.publish(str(self.config["MQTT"]["topic_pylips"]),
                                      json.dumps({"status": {"muted": muted, "volume": volume}}), retain=False)
